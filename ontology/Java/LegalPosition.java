...
public class LegalPosition extends Resource
{
  ...
  public LegalPosition(LegalSituation aAntecedent, LegalSituation aConsequent, StateTransition aStateTransition, Contract aContract, Role aDebtor, Role aCreditor, Role... allControllers)
  {
    ...
    if (!didAddAntecedent)
    {
      throw new RuntimeException("Unable to create antecedentOf due to antecedent. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
    ...
    if (!didAddContract)
    {
      throw new RuntimeException("Unable to create legalPosition due to contract. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
    boolean didAddDebtor = setDebtor(aDebtor);
    if (!didAddDebtor)
    {
      throw new RuntimeException("Unable to create debt due to debtor. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
    boolean didAddCreditor = setCreditor(aCreditor);
    if (!didAddCreditor)
    {
      throw new RuntimeException("Unable to create credit due to creditor. See https://manual.umple.org?RE002ViolationofAssociationMultiplicity.html");
    }
  }

  public LegalPosition(LegalSituation aAntecedent, LegalSituation aConsequent, Role... allControllersForStateTransition, Contract aContract, Role aDebtor, Role aCreditor, Role... allControllers)
  {
    super(allControllers);
    performer = new ArrayList<Party>();
    liable = new ArrayList<Party>();
  ...
  public void delete()
  {
    ...
    {
      existingStateTransition.delete();
    }
